...
void tersineCevir(dugum** head) {
	struct dugum* tmp = *head;
	int uzunluk = 0;
	for (; tmp != NULL; tmp = tmp->adres, uzunluk++);
	dugum** dugumler = (dugum*)malloc(sizeof(dugum*) * uzunluk);
	tmp = *head;
	int i = 0;
	for (i = 0; i < uzunluk; ++i) {
		dugumler[i] = tmp;
	...
	*head = dugumler[i - 1];
	for (i = uzunluk - 1; i > 0; i--) {
		dugumler[i]->adres = dugumler[i-1];
	}
	dugumler[i]->adres = NULL;
}
int main() {
